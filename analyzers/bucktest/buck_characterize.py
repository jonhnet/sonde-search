...
def parse_range(spec: str) -> list[float]:
    ...
    parts = spec.split(":")
    if len(parts) != 3:
        raise argparse.ArgumentTypeError(f"range must be start:stop:step, got '{spec}'")
...
def measure(
    g: gpp4323.GPP4323, channel: int, dmm: Optional[dmmlib.Keysight34465A] = None
) -> Measurement:
...
def run_iq_sweep(
    g: gpp4323.GPP4323, dmm: Optional[dmmlib.Keysight34465A], args: argparse.Namespace, writer
) -> None:
    ...
    for vin in args.iq_vin_list:
        ...
        print(f"  Vin={vin:6.2f} V  ->  Vin_meas={m.voltage:6.3f} V  " f"Iq={iq_ma:10.6f} mA")
        writer.writerow(
            {
                "test": "iq",
                "timestamp": now_iso(),
                "vin_set": f"{vin:.4f}",
                "iload_set_a": "",
                "vin_meas": f"{m.voltage:.6f}",
                "iin_meas_a": f"{m.current:.9f}",
                "pin_w": f"{m.power:.9f}",
                "vout_meas": "",
                "iout_meas_a": "",
                "pout_w": "",
...
def run_efficiency_sweep(
    ...
    for vin in args.eff_vin_list:
        ...
        for iload in args.eff_loads_list:
            ...
            print(
                f"  Vin={vin:6.2f} V  Iload={iload * 1000:7.1f} mA  ->  "
            ...
            writer.writerow(
                {
                    ...
                    "timestamp": now_iso(),
                    "vin_set": f"{vin:.4f}",
                    "iload_set_a": f"{iload:.4f}",
                    "vin_meas": f"{pin.voltage:.4f}",
                    "iin_meas_a": f"{pin.current:.6f}",
                    "pin_w": f"{pin.power:.6f}",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Characterize a buck converter (Iq and efficiency) " "with a GPP4323."
    )
    parser.add_argument("--host", default="gpp4323", help="GPP4323 hostname/IP (default: gpp4323)")
    parser.add_argument("--port", type=int, default=1026, help="TCP port (default: 1026)")
    parser.add_argument(
        "--in-channel", type=int, default=1, help="Channel sourcing the buck input (default: 1)"
    )
    parser.add_argument(
        "--load-channel",
        type=int,
        default=2,
        help="Channel loading the buck output, CC mode; " "must be 1 or 2 (default: 2)",
    )
    parser.add_argument(
        "--iq-vin",
        default="6:18:1",
        help="Iq input-voltage sweep start:stop:step " "(default: 6:18:1)",
    )
    parser.add_argument(
        "--eff-vin",
        default="8:18:2",
        help="Efficiency family input voltages start:stop:step " "(default: 8:18:2)",
    )
    parser.add_argument(
        "--eff-loads",
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Source current limit on the input channel, A " "(default: 2.0)",
    )
    parser.add_argument(
        "--settle",
        type=float,
        default=2.0,
        help="Seconds to settle after a setpoint change " "(default: 2.0)",
    )
    parser.add_argument(
        "--dmm-aperture",
        type=float,
    ...
    parser.add_argument("--skip-iq", action="store_true", help="Skip the Iq sweep")
    parser.add_argument("--skip-efficiency", action="store_true", help="Skip the efficiency sweep")
    ...
    try:
        ...
        if dmm:
            ...
            print(
                f"Measuring input current with DMM at {args.dmm_host} "
