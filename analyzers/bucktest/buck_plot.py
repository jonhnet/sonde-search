...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot buck characterization CSV with Plotly.")
    parser.add_argument("input", help="Input CSV")
    parser.add_argument("--iq-out", help="Iq plot PNG output " "(default: <input basename>_iq.png)")
    parser.add_argument(
        "--eff-out", help="Efficiency plot PNG output " "(default: <input basename>_efficiency.png)"
    )
    parser.add_argument(
        "--show",
        action="store_true",
        help="Open plots in a browser instead of only writing " "HTML files",
    )
    parser.add_argument(
        "--min-load-ma",
        type=float,
        default=0.0,
        help="Drop efficiency points below this load current, " "mA (default: 0, plot all)",
    )
    args = parser.parse_args()

    input_path = Path(args.input)
    iq_out = args.iq_out or input_path.with_name(f"{input_path.stem}_iq.png")
    eff_out = args.eff_out or input_path.with_name(f"{input_path.stem}_efficiency.png")
