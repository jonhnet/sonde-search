        ...
        while True:
            try:
                ...
                if tries >= self.MAX_SONDEHUB_TRIES:
                    print(
                        f"Couldn't get sondehub data, even after {self.MAX_SONDEHUB_TRIES} tries!"
                    )
                    raise e

                print(
                    f"Failure {tries} retrieving data from Sondehub ({e}); retrying after a short sleep..."
                )
...
class FakeSondeHub(SondeHubRetrieverBase):
    def __init__(self, filename):
        ...
        if "singlesonde" in filename:
            ...
            self._data = {serial: {rec["datetime"]: rec for rec in self._data}}
